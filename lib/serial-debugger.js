...
function SerialDebugger(client) {
  EventEmitter.call(this)
  this.mqttSerial = new MqttSerial({
    client: client || mqttClient
  , transmitTopic: 'upload/out'
  , receiveTopic: 'upload/in'
  })
}
util.inherits(SerialDebugger, EventEmitter)

SerialDebugger.prototype.start = function (portName, baudrate, cb) {
...
SerialDebugger.prototype.startUpload = function (portName, baudrate) {
  var self = this
  this.start(portName, baudrate, function () {
    self.reset(function () {
      self.emit('started')
    })
  })
